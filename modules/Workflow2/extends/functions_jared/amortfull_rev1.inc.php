...
if (!function_exists("wf_generate_table_amortfull")) {
    ...
    function wf_generate_table_amortfull(
        ...
        for ($ii = 1; $ii <= $term; $ii++) {
            ...
            $months[$ii]['payment_due'] = number_format(wf_pmt2($APR, $term, $loan), 2, '.', '');

            // payment_principle
            // $months[$ii]['payment_principle'] = wf_ppmt($APR, $ii, $term, -$loan);
            $months[$ii]['payment_principle'] = number_format(wf_ppmt($APR, $ii, $term, -$loan), 2, '.', '');

            // payment_interest
            // $months[$ii]['payment_interest'] = wf_ipmt($APR, $ii, $term, -$loan);
            $months[$ii]['payment_interest'] = number_format(wf_ipmt($APR, $ii, $term, -$loan), 2, '.', '');

            // payment_gst
            // $months[$ii]['payment_gst'] = $months[$ii]['payment_principle'] * ($GST / 100);
            $months[$ii]['payment_gst'] = number_format($months[$ii]['payment_principle'] * ($GST / 100), 2, '.', '');

            // new_balance
            if ($ii == 1) {
                // $months[$ii]['new_balance'] = $loan - $months[$ii]['payment_principle'];
                $months[$ii]['new_balance'] = number_format($loan - $months[$ii]['payment_principle'], 2, '.', '');
            } else {
                // $months[$ii]['new_balance'] = $months[$ii - 1]['new_balance'] - $months[$ii]['payment_principle'];
                $months[$ii]['new_balance'] = number_format($months[$ii - 1]['new_balance'] - $months[$ii]['payment_principle'], 2, '.', '');
            }

            // payment_total
            $months[$ii]['payment_total'] = number_format($months[$ii]['payment_interest'] + $months[$ii]['payment_principle'] + $months[$ii]['payment_gst'], 2, '.', '');
        ...
        try {
            ...
            for ($ii = 1; $ii <= count($months); $ii++) {
                $stmt->execute(array(
                    ...
                    ':payment_due'       => $months[$ii]['payment_due'],
                    ':payment_principle' => $months[$ii]['payment_principle'],
                    ':payment_interest'  => $months[$ii]['payment_interest'],
                    ':payment_gst'       => $months[$ii]['payment_gst'],
                    ':new_balance'       => $months[$ii]['new_balance'],
                    ':payment_total'     => $months[$ii]['payment_total']
                ));
