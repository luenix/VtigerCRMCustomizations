...
class SMSNotifier extends SMSNotifierBase
{
    ...
    static function sendsms($message, $tonumbers, $ownerid = false, $linktoids = false, $linktoModule = false)
    {
        ...
        $focus->column_fields['direction'] = 'Outbound';
        $focus->column_fields['tonumber'] = $tonumbers;
        //$focus->column_fields['related_to'] = $linktoids; Sets the record ID but goes into a loop and fails to send SMS
        ...
        if ($linktoids !== false) {
            ...
            if ($linktoModule !== false) {
                ...
                // sleep 3
                // call getSMSStatusInfo($focus->id)
                // Update queries to touch 'To Number', 'SMS Status', and 'Related To'
            } else {
    ...
    static function fireSendSMS($message, $tonumbers)
    {
        ...
        $provider = SMSNotifierManager::getActiveProviderInstance();
        if ($provider) {
            return $provider->send($message, $numbers);
        }
    }
